...
CPathFinderRay::~CPathFinderRay() { Clear(); }
...
void CPathFinderRay::TurnAttach(int &rAttach, int iDirection) {
  rAttach += iDirection;
  if (rAttach > PF_Crawl_Left)
    rAttach = PF_Crawl_Top;
  if (rAttach < PF_Crawl_Top)
    rAttach = PF_Crawl_Left;
}
...
int CPathFinderRay::FindCrawlAttach(int iX, int iY) {
  if (!PointFree(iX, iY - 1))
    return PF_Crawl_Top;
  if (!PointFree(iX, iY + 1))
    return PF_Crawl_Bottom;
  if (!PointFree(iX - 1, iY))
    return PF_Crawl_Left;
  if (!PointFree(iX + 1, iY))
    return PF_Crawl_Right;
...
BOOL CPathFinder::Find(int iFromX, int iFromY, int iToX, int iToY, BOOL (*fnSetWaypoint)(int, int, int), int iWaypointParameter) {
  ...
  if (!AddRay(iFromX, iFromY, iToX, iToY, 0, PF_Direction_Left, NULL))
    return FALSE;
  if (!AddRay(iFromX, iFromY, iToX, iToY, 0, PF_Direction_Right, NULL))
    return FALSE;
