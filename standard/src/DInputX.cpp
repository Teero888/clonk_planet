...
BOOL InitDirectInput(HINSTANCE g_hinst, HWND hwnd, int resx, int resy) {
  ...
  if (FAILED(hr)) {
    Log("CreateDevice(SysMouse) failure");
    return FALSE;
  }

  hr = g_pMouse->SetDataFormat(&c_dfDIMouse);
  if (FAILED(hr)) {
    Log("SetDataFormat(SysMouse, dfDIMouse) failure");
    return FALSE;
  }

  hr = g_pMouse->SetCooperativeLevel(hwnd, DISCL_EXCLUSIVE | DISCL_FOREGROUND);
  if (FAILED(hr)) {
    Log("SetCooperativeLevel(SysMouse) failure");
    return FALSE;
  }

  g_hevtMouse = CreateEvent(0, 0, 0, 0);
  if (g_hevtMouse == NULL) {
    Log("CreateEvent failure");
    return FALSE;
  }

  hr = g_pMouse->SetEventNotification(g_hevtMouse);
  if (FAILED(hr)) {
    Log("SetEventNotification(SysMouse)");
    return FALSE;
  }
  ...
  if (g_pMouse)
    if (fActive) {
      if (g_pMouse->Acquire() == DI_OK)
        Log("DirectInput: sync acquired");
      else
        Log("DirectInput: sync acquire failure");
    } else {
      g_pMouse->Unacquire();
      Log("DirectInput: sync released");
    }
}

void DirectInputCritical() {
  // Test for mouse input
  if (WaitForSingleObject(g_hevtMouse, 0) != WAIT_OBJECT_0)
    return;
  ...
  while (TRUE) {
    ...
    case DIMOFS_X:
      DIMouseUpdatePosition(od.dwData, 0);
      break;
    // Mouse vertical motion
    case DIMOFS_Y:
      DIMouseUpdatePosition(0, od.dwData);
      break;
    // Button 0 pressed or released
    case DIMOFS_BUTTON0:
      MouseB0 = (od.dwData & 0x80);
      break;
    // Button 1 pressed or released
    case DIMOFS_BUTTON1:
      MouseB1 = (od.dwData & 0x80);
      break;
    // Button 1 pressed or released
    case DIMOFS_BUTTON2:
      MouseB2 = (od.dwData & 0x80);
      break;
    }
  }
}
