...
const int CBlack = 0, CGray1 = 1, CGray2 = 2, CGray3 = 3, CGray4 = 4,
          CGray5 = 5, CWhite = 6, CDRed = 7, CDGreen = 8, CDBlue = 9, CRed = 10,
          CGreen = 11, CLBlue = 12, CYellow = 13, CBlue = 14;
...
public:
  // General
  BOOL Init(HWND hWnd, BOOL Fullscreen, int iResX, int iResY,
            BOOL fUsePageLock);
  void Clear();
  void Default();
  BOOL PageFlip();
  ...
  // Surface
  BYTE *LockSurface(SURFACE sfcSurface, int &lPitch, int *lpSfcWdt = 0,
                    int *lpSfcHgt = 0);
  ...
  void SurfaceShiftColorRange(SURFACE sfcSfc, int iRngLo, int iRngHi,
                              int iShift);
  void SurfaceAllowColor(SURFACE sfcSfc, BYTE iRngLo, BYTE iRngHi,
                         BOOL fAllowZero = FALSE);
  // Blit
  BOOL BlitFast(SURFACE sfcSource, int fx, int fy, SURFACE sfcTarget, int tx,
                int ty, int wdt, int hgt);
  ...
  BOOL BlitSurface2Window(SURFACE sfcSource, int fX, int fY, int fWdt, int fHgt,
                          HWND hWnd, int tX, int tY, int tWdt, int tHgt);
  // Text
  ...
  void DrawBoxColorTable(SURFACE sfcDest, int x1, int y1, int x2, int y2,
                         BYTE *bypColorTable);
  ...
  void DrawInline(SURFACE sfcDest, int iX1, int iY1, int iX2, int iY2,
