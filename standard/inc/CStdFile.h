...
public:
  ...
  BOOL Close();
  BOOL Read(void *pBuffer, int iSize, BOOL fPacked = FALSE,
            int *ipActual = NULL);
  BOOL Write(void *pBuffer, int iSize, BOOL fPacked = FALSE,
             int *ipFSize = NULL);
  BOOL WriteString(const char *szStr);
  BOOL Rewind();
  BOOL Advance(int iOffset, BOOL fPacked = FALSE);
  BOOL Save(const char *szFilename, BYTE *pBuffer, int iSize,
            BOOL fCompressed = FALSE);
  BOOL Load(const char *szFilename, BYTE **ppBuffer, int *ipSize,
            int iExtra = 0, BOOL fCompressed = FALSE);
