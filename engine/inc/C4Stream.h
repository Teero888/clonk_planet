...
const int C4STRM_Ok = 0, C4STRM_NoGood = 1, C4STRM_NoPacket = 2,
          C4STRM_NoData = 3, C4STRM_NoMemory = 4, C4STRM_DataError = 5,
          C4STRM_SendError = 6, C4STRM_ReceiveError = 7, C4STRM_OutOfOrder = 8,
          C4STRM_NoOpen = 9, C4STRM_Break = 10, C4STRM_WrongPacket = 11,
          C4STRM_NoFile = 12, C4STRM_DataMismatch = 13, C4STRM_NoSocket = 14,
          C4STRM_InvalidParameters = 15, C4STRM_NoListen = 16;
...
public:
  ...
  int SendFile(const char *szFilename, int iType = C4PK_File,
               void (*fnPrcs)(int) = NULL);
  int ReceivePacket(int iType, C4Packet &rPacket);
  int Connect(const char *szName, int iRole, const char *szAddress = NULL,
              C4Stream **ppAccept = NULL, int iPort = C4PORT_Control);
