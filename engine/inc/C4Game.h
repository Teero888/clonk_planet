...
public:
  ...
  BOOL JoinPlayer(const char *szFilename, int iAtClient,
  ...
  C4Object *CreateObject(C4ID type, int owner = NO_OWNER, int x = 50,
                         int y = 50, int r = 0, FIXED xdir = 0, FIXED ydir = 0,
                         FIXED rdir = 0);
  C4Object *CreateObjectConstruction(C4ID type, int owner, int ctx = 0,
                                     int bty = 0, int con = 1,
                                     BOOL terrain = FALSE);
  C4Object *CreateInfoObject(C4ObjectInfo *cinf, int owner, int tx = 50,
                             int ty = 50);
  ...
  C4Object *FindObject(C4ID id, int x = 0, int y = 0, int wdt = 0, int hgt = 0,
                       DWORD ocf = OCF_All, const char *szAction = NULL,
                       C4Object *pActionTarget = NULL,
                       C4Object *pExclude = NULL, C4Object *pContainer = NULL,
                       int iOwner = ANY_OWNER, C4Object *pFindNext = NULL);
  int ObjectCount(C4ID id, int x = 0, int y = 0, int wdt = 0, int hgt = 0,
                  DWORD ocf = OCF_All, const char *szAction = NULL,
  ...
  C4Object *FindObjectByCommand(int iCommand, C4Object *pTarget = NULL,
                                int iTx = 0, int iTy = 0,
                                C4Object *pTarget2 = NULL);
  void CastObjects(C4ID id, int num, int level, int tx, int ty,
                   int iOwner = NO_OWNER);
  void BlastCastObjects(C4ID id, int num, int tx, int ty);
  C4Object *PlaceVegetation(C4ID id, int iX, int iY, int iWdt, int iHgt,
                            int iGrowth);
...
protected:
  ...
  C4Object *NewObject(C4Def *ndef, int owner, C4ObjectInfo *info, int tx,
                      int ty, int tr, FIXED xdir, FIXED ydir, FIXED rdir,
                      int con);
...
const int C4RULE_StructuresNeedEnergy = 1, C4RULE_ConstructionNeedsMaterial = 2,
