...
class C4Game
  ...
  public:
    ...
    C4Object *CreateObject(C4ID type, int owner=NO_OWNER,
                           int x=50, int y=50, int r=0,
                           FIXED xdir=0, FIXED ydir=0, FIXED rdir=0);
    C4Object *CreateObjectConstruction(C4ID type, 
                                       int owner,
                                       int ctx=0, int bty=0,
                                       int con=1, BOOL terrain=FALSE);
    C4Object *CreateInfoObject(C4ObjectInfo *cinf, int owner, 
                               int tx=50, int ty=50);
    ...
    C4Object *AtObject(int ctx, int cty, DWORD &ocf, 
                       C4Object *exclude=NULL);
    C4Object *OverlapObject(int tx, int ty, int wdt, int hgt, 
                            int category);
    C4Object *FindObject(C4ID id, 
												 int x=0, int y=0, int wdt=0, int hgt=0,
												 DWORD ocf=OCF_All,
  ...
  protected:
    ...
    C4Object *NewObject( C4Def *ndef,
                         int owner, C4ObjectInfo *info,
