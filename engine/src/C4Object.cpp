...
BOOL C4Object::Init(C4Def *pDef, 
										int iOwner, C4ObjectInfo *pInfo,
										int nx, int ny, int nr,
  ...
  if (*lpSfc) 
    if (!GetSurfaceSize(*lpSfc,swdt,shgt)) 
      ...
      if ((wdt!=swdt) || (hgt!=shgt))
        { DestroySurface(*lpSfc); *lpSfc=NULL; }  
  ...
  if (Con>=FullCon)
    ...
    // Rotated
      ...
      Engine.DDraw.BlitRotate(Def->Bitmap[Color],
             0,0,Def->Shape.Wdt,Def->Shape.Hgt,
             cBitmap,
             (Shape.Wdt-Def->Shape.Wdt)/2,
             (Shape.Hgt-Def->Shape.Hgt)/2,
             Def->Shape.Wdt,Def->Shape.Hgt,
             r*100);
  ...
  // Under construction
    ...
    if (Def->GrowthType)
      ...
      if (!Def->Rotateable || (r==0))
        ...
        Engine.DDraw.Blit(Def->Bitmap[Color],
              0,0,Def->Shape.Wdt,Def->Shape.Hgt,
              cBitmap,
              0,0,Shape.Wdt,Shape.Hgt);
        BaseFace.Set(cBitmap,0,0,Shape.Wdt,Shape.Hgt);
        }
      // Rotated: Rotated stretched cBitmap
        ...
        Engine.DDraw.BlitRotate(Def->Bitmap[Color],
               ...
               Def->Shape.Wdt*Con/FullCon,
               Def->Shape.Hgt*Con/FullCon,
               r*100);
        BaseFace.Set(cBitmap,0,0,Shape.Wdt,Shape.Hgt);        
        }
      }
    // Construction Type Display
      ...
      if (!Def->Rotateable || (r==0))
        {
        RecheckSurface(&(cBitmap));
        BaseFace.Set(Def->Bitmap[Color],
                 0,Def->Shape.Hgt-Shape.Hgt,
...
void C4Object::ComponentConCutoff()
  ...
  for (cnt=0; Component.GetID(cnt); cnt++)
    Component.SetCount(cnt,
      Min(Component.GetCount(cnt),Def->Component.GetCount(cnt)*Con/FullCon));
  }
...
void C4Object::ComponentConGain()
  ...
  for (cnt=0; Component.GetID(cnt); cnt++)
    Component.SetCount(cnt,
      Max(Component.GetCount(cnt),Def->Component.GetCount(cnt)*Con/FullCon));
  }
  ...
  if (!Tick5)
    ...
    if (MatValid(mat=GBackMat(x,y)))
      ...
      if (Game.Material.Map[mat].Extinguisher)
        { Extinguish(); if (GBackLiquid(x,y)) SoundEffect("Pshshsh",0,100,this); }
			// Inflame
			if (!Random(3))				
...
BOOL C4Object::ActivateMenu(int iMenu, int iMenuSelect, 
														int iMenuData, int iMenuPosition, 
														C4Object *pTarget)
  {
	// Variables
	C4FacetEx fctSymbol;
        ...
        case C4CMD_MoveTo:
          ...
          ccx=pCom->Tx; ccy=pCom->Ty;
			...
			if (szCommand[0])
				...
				if (iMoveTos) { SNewSegment(OSTR,"|"); sprintf(OSTR+SLen(OSTR),"%dx MoveTo",iMoveTos); iMoveTos=0; }
				// Current message
...
int C4Object::CallControl(BYTE byCom,
...
void C4Object::DrawCommand(C4Facet &cgoBar, int iAlign, const char *szFunctionFormat, 
	...
	if (!fFlash || Tick35>15)
		DrawCommandKey(cgoLeft,iCom,FALSE,
									 Config.Graphics.ShowCommandKeys ? PlrControlKeyName(iPlayer,Com2Control(iCom)) : NULL );
    ...
    switch (iProcedure)
      ...
      case DFA_SCALE: 
        ...
        if (pPhysical->CanHangle) if (Action.ComDir==COMD_Up)
          ...
          if (Action.Dir==DIR_Left) { iDir=DIR_Right; iComDir=COMD_Right; }
