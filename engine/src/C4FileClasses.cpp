...
BOOL SetC4FileClasses(const char *szEnginePath, const char *szFrontendPath)
  ...
  if (!SetRegFileClass("Clonk4.Executable","c4x","Clonk 4 Engine",szEnginePath,5,C4FileClassContentType)) return FALSE;
  ...
  if (!SetRegFileClass("Clonk4.Material","c4m","Clonk 4 Material",szEnginePath,8,C4FileClassContentType)) return FALSE;
  if (!SetRegFileClass("Clonk4.Binary","c4b","Clonk 4 Binary",szEnginePath,9,C4FileClassContentType)) return FALSE;
  if (!SetRegFileClass("Clonk4.Video","c4v","Clonk 4 Video",szEnginePath,10,"video/avi")) return FALSE;
	...
	if (!SetProtocol("clonkplanet","%s /Net",szFrontendPath)) return FALSE;
