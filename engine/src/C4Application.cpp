...
static void glfw_key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        ...
        if (vk) Game.KeyboardInput(vk, (mods & GLFW_MOD_ALT) != 0);
    }
}
...
static void glfw_mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT) {
        if (action == GLFW_PRESS) c4button = C4MC_Button_LeftDown;
        else if (action == GLFW_RELEASE) c4button = C4MC_Button_LeftUp;
    } else if (button == GLFW_MOUSE_BUTTON_RIGHT) {
        if (action == GLFW_PRESS) c4button = C4MC_Button_RightDown;
        else if (action == GLFW_RELEASE) c4button = C4MC_Button_RightUp;
    }
...
int C4Application::HandleMessage()
{
    ...
    if (window) {
        if (glfwWindowShouldClose(window)) return 2;
