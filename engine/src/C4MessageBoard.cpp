...
BOOL C4MessageBoard::Out(const char *szText)
	...
	// Starts with /me?
	if(SEqual2(szText, "/me")) sprintf(szBuffer,"* %s %s", pPlr ? pPlr->Name : Game.Network.LocalName ,szText+4);
	else sprintf(szBuffer,"%s: %s", pPlr ? pPlr->Name : Game.Network.LocalName ,szText);		
