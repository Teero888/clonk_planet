...
static bool IsPointerValid(const void* ptr) {
    if (!ptr) return false;
    if ((uintptr_t)ptr < 65536) return false;
    if (g_null_fd == -1) g_null_fd = open("/dev/null", O_WRONLY);
    if (g_null_fd == -1) return true;
...
static bool IsObjectValid(C4Object* pObj) {
    if (!IsPointerValid(pObj)) return false;
...
const char *FnStringFormat(const char *szFormatPar, long iPar0=0, long iPar1=0, long iPar2=0, long iPar3=0, long iPar4=0, long iPar5=0, long iPar6=0, long iPar7=0, long iPar8=0, long iPar9=0)
	{
...
long Fn_while(C4Thread *cthr, long fCondition)
  ...
  cthr->NextStatementAdjacent=TRUE;
	// Condition false, skip next statement
	if (!fCondition)
		{ cthr->SkipNextStatement=TRUE; return FALSE; }
	// Condition true, jump back to while statement at next statement separator
	cthr->JumpbackStatement=TRUE;
	return TRUE;
	}

long FnChangeDef(C4Thread *cthr, C4ID to_id, C4Object *pObj)
	{
	if (!pObj) pObj=cthr->cObj; if (!IsObjectValid(pObj)) return FALSE;
...
long FnGrabContents(C4Thread *cthr, C4Object *from)
  {
  if (!cthr->cObj) return FALSE;
  if (!IsObjectValid(from)) return FALSE;
  C4Object *thing;
...
long FnFling(C4Thread *cthr, C4Object *pObj, long iXDir, long iYDir)
  {
  if (!IsObjectValid(pObj)) return FALSE;
...
long FnExit(C4Thread *cthr, C4Object *pObj, 
					 int tx, int ty, int tr, 
					 int txdir, int tydir, int trdir)
  {
  if (!IsObjectValid(pObj)) return FALSE;
  if (cthr->cObj) 
		{	tx+=cthr->cObj->x; ty+=cthr->cObj->y;	}
...
long FnSetAction(C4Thread *cthr, const char *szAction, 
...
long FnObjectSetAction(C4Thread *cthr, C4Object *pObj, const char *szAction, 
											C4Object *pTarget, C4Object *pTarget2)
  {
  if (!szAction || !IsObjectValid(pObj)) return FALSE;
...
long FnSetCommand(C4Thread *cthr, C4Object *pObj, 
  ...
  {
	// Object
	if (!pObj) pObj=cthr->cObj; if (!IsObjectValid(pObj) || !szCommand) return FALSE;
	// Command
...
long FnAddCommand(C4Thread *cthr, C4Object *pObj, 
									const char *szCommand, 
									C4Object *pTarget, int iTx, int iTy,
									C4Object *pTarget2,
									int iUpdateInterval,
...
long FnAppendCommand(C4Thread *cthr, C4Object *pObj, 
										 const char *szCommand, 
										 C4Object *pTarget, int iTx, int iTy,
										 C4Object *pTarget2,
										 int iUpdateInterval,
...
const char *FnGetName(C4Thread *cthr, C4Object *pObj, C4ID idDef)
  {
	...
	// Object name
	if (!pObj) pObj=cthr->cObj; if (!IsObjectValid(pObj)) return NULL;
	return pObj->GetName();
  }
...
long FnGetEnergy(C4Thread *cthr, C4Object *pObj)
  { 
	if (!pObj) pObj=cthr->cObj; if (!IsObjectValid(pObj)) return 0;
...
long FnGetOwner(C4Thread *cthr, C4Object *pObj)
  {
	if (!pObj) pObj=cthr->cObj; if (!IsObjectValid(pObj)) return -1;
  return pObj->Owner;
  }
...
long FnGetCategory(C4Thread *cthr, C4Object *pObj, C4ID idDef)
  {
	// Def category
	C4Def *pDef;
...
long FnGetValue(C4Thread *cthr, C4Object *pObj, C4ID idDef)
  {
	// Def value
	C4Def *pDef;
...
long FnValue(C4Thread *cthr, C4ID id)
  {
...
C4ID FnGetID(C4Thread *cthr, C4Object *pObj)
  {
  if (!pObj) pObj=cthr->cObj; if (!IsObjectValid(pObj)) return C4ID_None;
  return pObj->Def->id;
  }
...
long FnCreateMenu(C4Thread *cthr, int iSymbol, C4Object *pMenuObj, C4Object *pCommandObj,
																 int iExtra, const char *szCaption, int iExtraData,
																 int iStyle, BOOL fPermanent)
	{
  if (!pMenuObj) pMenuObj=cthr->cObj; if (!IsObjectValid(pMenuObj)) return FALSE;
  if (!pCommandObj) pCommandObj=cthr->cObj; if (!IsObjectValid(pCommandObj)) return FALSE;
...
long FnAddMenuItem(C4Thread *cthr, const char *szCaption, const char *szCommand, 
									C4ID idItem, C4Object *pMenuObj,
									int iCount, int iParameter, const char *szInfoCaption)
	{
  if (!pMenuObj) pMenuObj=cthr->cObj; if (!IsObjectValid(pMenuObj)) return FALSE;
...
long FnContentsCount(C4Thread *cthr, C4ID id, C4Object *pObj)
  {
  if (!pObj) pObj=cthr->cObj; if (!IsObjectValid(pObj)) return 0;
...
long FnComponentAll(C4Thread *cthr, C4Object *pObj, C4ID c_id)
  {
  int cnt;
  if (!IsObjectValid(pObj)) return FALSE;
...
C4Object *FnCreateConstruction(C4Thread *cthr, 
            C4ID id, int iXOffset, int iYOffset, int iOwner,
...
long FnFindConstructionSite(C4Thread *cthr, C4ID id, long iVarX, long iVarY)
	{
...
long FnObjectCount( C4Thread *cthr,
									 C4ID id, 
...
C4Object *FnFindObject(C4Thread *cthr,
											 C4ID id, 
   										 int x, int y, int wdt, int hgt,
										   int dwOCF,
											 const char *szAction, C4Object *pActionTarget,
...
long FnMakeCrewMember(C4Thread *cthr, C4Object *pObj, long iPlayer)
	{
	if (!IsObjectValid(pObj) || !ValidPlr(iPlayer)) return FALSE;
...
long FnObjectInsertMaterial(C4Thread *cthr, long mat, C4Object *target)
	{
	if (!IsObjectValid(target)) return FALSE;
...
long FnObjectExtractLiquid(C4Thread *cthr, C4Object *from)
	{
	if (!IsObjectValid(from)) return MNone;
	...
	for (clnk=from->Contents.First; clnk && (pObj=clnk->Obj); clnk=clnk->Next)
	 if (pObj->Status)
		{
		if (pObj->Def->id==C4Id("WBRL")) if (pObj->BarrelFill>0)
			{ pObj->BarrelFill--; if (pObj->BarrelFill<1) pObj->ChangeDef(C4Id("BARL"));
				return Game.Material.Get("Water"); }
		if (pObj->Def->id==C4Id("ABRL")) if (pObj->BarrelFill>0)
			{ pObj->BarrelFill--; if (pObj->BarrelFill<1) pObj->ChangeDef(C4Id("BARL"));
				return Game.Material.Get("Acid"); }
		if (pObj->Def->id==C4Id("OBRL")) if (pObj->BarrelFill>0)
			{ pObj->BarrelFill--; if (pObj->BarrelFill<1) pObj->ChangeDef(C4Id("BARL"));
				return Game.Material.Get("Oil"); }
		if (pObj->Def->id==C4Id("LBRL")) if (pObj->BarrelFill>0)
			{ pObj->BarrelFill--; if (pObj->BarrelFill<1) pObj->ChangeDef(C4Id("BARL"));
...
long FnDoHomebaseMaterial(C4Thread *cthr, long iPlr, C4ID id, long iChange)
	{
  if (!ValidPlr(iPlr)) return FALSE;
...
long FnSetPlrKnowledge(C4Thread *cthr, long iPlr, C4ID id)
  {
  if (!ValidPlr(iPlr)) return FALSE;
...
long FnGetPlrMagic(C4Thread *cthr, long iPlr, C4ID id, long iIndex)
  {
  if (!ValidPlr(iPlr)) return FALSE;
	// Search by id, check if available, return BOOL
...
long FnGetPlayerCount(C4Thread *cthr)
  {
...
long FnGetTemperature(C4Thread *cthr)
  {
...
long FnGetSeason(C4Thread *cthr)
  {
...
long FnGetClimate(C4Thread *cthr)
  {
...
long FnLandscapeWidth(C4Thread *cthr)
	{
	return GBackWdt;
	}

long FnLandscapeHeight(C4Thread *cthr)
	{
	return GBackHgt;
	}

long FnLaunchLightning(C4Thread *cthr, long x, long y, long xdir, long xrange, long ydir, long yrange)
	{
...
long FnNot(C4Thread *cthr, long fCondition)
  {
  return !fCondition;
  } 

long FnOr(C4Thread *cthr, long fCon1, long fCon2, long fCon3, long fCon4, long fCon5)
  {
  return (fCon1 || fCon2 || fCon3 || fCon4 || fCon5);
  } 

long FnAnd(C4Thread *cthr, long fCon1, long fCon2)
  {
  return (fCon1 && fCon2);
  } 
...
long FnAbs(C4Thread *cthr, long iVal)
  {
  return Abs(iVal);
  }

long FnMul(C4Thread *cthr, long iVal1, long iVal2)
  {
  return (iVal1*iVal2);
  }
...
long FnSqrt(C4Thread *cthr, long iValue)
	{
...
long FnMin(C4Thread *cthr, long iVal1, long iVal2)
  {  
  return Min(iVal1,iVal2);
  }

long FnMax(C4Thread *cthr, long iVal1, long iVal2)
  {  
...
long FnObjectDistance(C4Thread *cthr, C4Object *pObj2, C4Object *pObj)
	{
	if (!pObj) pObj=cthr->cObj; if (!IsObjectValid(pObj) || !IsObjectValid(pObj2)) return 0;
...
long FnRandom(C4Thread *cthr, long iRange)
  {
  return Random(iRange);
  }

long FnSafeRandom(C4Thread *cthr, long iRange)
  {
...
long FnDefinitionCall(C4Thread *cthr, 
										 C4ID idID, const char *szFunction,
										 int par0, int par1, int par2, int par3, int par4,
										 int par5, int par6, int par7, int par8, int par9)
  {
