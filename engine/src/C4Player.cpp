...
void C4Player::DrawHostility(C4FacetEx &cgo, int iIndex)
{
	...
	if (pPlr = Game.Players.GetByIndex(iIndex))
	{
		...
		// Hostility flag
		if (pPlr != this)
			Game.GraphicsResource.fctFlag.DrawX(cgo.Surface,
												cgo.X, cgo.Y + cgo.Hgt / 3, 2 * cgo.Wdt / 3, 2 * cgo.Hgt / 3,
...
BOOL C4Player::Compile(const char *szSource, const char *szSecExt)
{
	C4Compiler Compiler;
	DefaultRuntimeData();
	return Compiler.CompileStructure(C4CR_Player,
									 szSource,
									 this,
									 szSecExt);
}
...
BOOL C4Player::Decompile(char **ppOutput, int *ipSize, const char *szSecExt)
{
	C4Compiler Compiler;
	C4Player dC4P;
	return Compiler.DecompileStructure(C4CR_Player,
									   this,
									   &dC4P,
									   ppOutput,
									   ipSize,
