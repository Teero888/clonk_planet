...
/* Notes
   ...
   09-30-99
         ...
         10-26-99
         C4PathFinderRay::Crawl use IsCrawlAttach instead of GetCrawlAttach
   (which might not correctly indicate attach loss). Otherwise 1 pixel clefts
   can lead to backward crawl looping. Surprised, I haven't noticed this before.
         Also do not check attach loss on first crawl for that be might
   diagonal. C4PF_Ray_Crawl try new ray: don't worry about checking backwards
   jump if path to target is all free. C4PathFinderRay::FindCrawlAttachDiagonal
   check according to desired direction or else might lead off to no attach.

         11-24-99
         TransferZones

         12-11-99
         SetCompletePath don't set move-to waypoint if setting use-zone waypoint
   (is done by C4Command::Transfer on demand and would only cause
   no-good-entry-point move-to's on crawl-zone-entries).
...
C4PathFinderRay::~C4PathFinderRay() { Clear(); }
...
BOOL C4PathFinderRay::Execute() {
  ...
  case C4PF_Ray_Launch:
    ...
    // Path intersected by solid
      ...
      CrawlLength = 0;
      if (!CrawlAttach)
        CrawlAttach = FindCrawlAttachDiagonal(X2, Y2, Direction);
  ...
  // - - - - - -
  case C4PF_Ray_Still:
  case C4PF_Ray_Failure:
  case C4PF_Ray_Deleted:
    return FALSE;
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // - - - - - - -
  }
...
/*void C4PathFinderRay::DrawLine(SURFACE sfcSurface, int rX, int rY, int iToX,
  int iToY, BYTE byCol)
        {
  int d,dx,dy,aincr,bincr,xincr,yincr,x,y;
        // Y based
  if (Abs(iToX-rX)<Abs(iToY-rY))
    ...
    for (y=rY; y!=iToY; y+=yincr)
      {
                        Engine.DDraw.SetPixel(sfcSurface,x,y,byCol);
...
BOOL C4PathFinderRay::Crawl() {
  ...
  // Last attach lost (don't check on first crawl for that might be a diagonal
  // attach)
  if (CrawlLength)
...
void C4PathFinderRay::CrawlByAttach(int &rX, int &rY, int iAttach, int iDirection) {
  ...
  case C4PF_Crawl_Top:
    rX += iDirection;
    break;
  case C4PF_Crawl_Bottom:
    rX -= iDirection;
    break;
  case C4PF_Crawl_Left:
    rY -= iDirection;
    break;
  case C4PF_Crawl_Right:
    rY += iDirection;
    break;
  }
}

void C4PathFinderRay::TurnAttach(int &rAttach, int iDirection) {
  rAttach += iDirection;
  if (rAttach > C4PF_Crawl_Left)
    rAttach = C4PF_Crawl_Top;
  if (rAttach < C4PF_Crawl_Top)
    rAttach = C4PF_Crawl_Left;
}

int C4PathFinderRay::FindCrawlAttach(int iX, int iY) {
  if (!PointFree(iX, iY - 1))
    return C4PF_Crawl_Top;
  if (!PointFree(iX, iY + 1))
    return C4PF_Crawl_Bottom;
  if (!PointFree(iX - 1, iY))
    return C4PF_Crawl_Left;
  if (!PointFree(iX + 1, iY))
    return C4PF_Crawl_Right;
  return C4PF_Crawl_NoAttach;
}

void C4PathFinderRay::CrawlToAttach(int &rX, int &rY, int iAttach) {
  switch (iAttach) {
  case C4PF_Crawl_Top:
    rY--;
    break;
...
BOOL C4PathFinderRay::IsCrawlAttach(int iX, int iY, int iAttach) {
  CrawlToAttach(iX, iY, iAttach);
  return !PointFree(iX, iY);
}

int C4PathFinderRay::FindCrawlAttachDiagonal(int iX, int iY, int iDirection) {
  // Going left
  if (iDirection == C4PF_Direction_Left) {
    // Top Left
    if (!PointFree(iX - 1, iY - 1))
      return C4PF_Crawl_Top;
    // Bottom left
    if (!PointFree(iX - 1, iY + 1))
      return C4PF_Crawl_Left;
...
C4PathFinder::~C4PathFinder() { Clear(); }
...
void C4PathFinder::Clear() {
  ...
  for (pRay = FirstRay; pRay; pRay = pNext) {
    pNext = pRay->Next;
    delete pRay;
  }
  FirstRay = NULL;
...
void C4PathFinder::Draw(C4FacetEx &cgo) {
  if (TransferZones)
    TransferZones->Draw(cgo);
  for (C4PathFinderRay *pRay = FirstRay; pRay; pRay = pRay->Next)
    pRay->Draw(cgo);
}

void C4PathFinder::Run() {
  if (TransferZones)
    TransferZones->ClearUsed();
  Success = FALSE;
...
BOOL C4PathFinder::Find(int iFromX, int iFromY, int iToX, int iToY, BOOL (*fnSetWaypoint)(int, int, int, int), int iWaypointParameter) {
  ...
  if (!PointFree(iFromX, iFromY) || !PointFree(iToX, iToY))
    return FALSE;

  // Add the first two rays
  if (!AddRay(iFromX, iFromY, iToX, iToY, 0, C4PF_Direction_Left, NULL))
    return FALSE;
  if (!AddRay(iFromX, iFromY, iToX, iToY, 0, C4PF_Direction_Right, NULL))
    return FALSE;
...
BOOL C4PathFinder::SplitRay(C4PathFinderRay *pRay, int iAtX, int iAtY) {
  // Max depth
  if (pRay->Depth >= C4PF_MaxDepth)
    return FALSE;
  // Allocate and set new ray
  C4PathFinderRay *pNewRay;
  if (!(pNewRay = new C4PathFinderRay))
    return FALSE;
  ...
  pNewRay->X2 = iAtX;
  pNewRay->Y2 = iAtY;
  pNewRay->TargetX = pRay->TargetX;
  pNewRay->TargetY = pRay->TargetY;
