    ...
    for (y=rY; y!=iToY; y+=yincr)
      {
			...
			if (ppZone) if (pPathFinder->TransferZones)
...
/*void C4PathFinderRay::DrawLine(SURFACE sfcSurface, int rX, int rY, int iToX, int iToY, BYTE byCol)
	{
  int d,dx,dy,aincr,bincr,xincr,yincr,x,y;
	// Y based
  if (Abs(iToX-rX)<Abs(iToY-rY))
...
BOOL C4PathFinder::AddRay(int iFromX, int iFromY, int iToX, int iToY, int iDepth, int iDirection, C4PathFinderRay *pFrom, C4TransferZone *pUseZone)
	{
	// Max depth
	if (iDepth>=C4PF_MaxDepth) return FALSE;
...
BOOL C4PathFinder::SplitRay(C4PathFinderRay *pRay, int iAtX, int iAtY)
	{
	// Max depth
	if (pRay->Depth>=C4PF_MaxDepth) return FALSE;
	// Allocate and set new ray
