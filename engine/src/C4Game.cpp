			...
			if (pPlr->CursorFlash || pPlr->SelectFlash)
				if (pPlr->Cursor)
					...
					if (Inside(cox,1-fctCursor.Wdt,cgo.Wdt) && Inside(coy,1-fctCursor.Hgt,cgo.Hgt))
						...
						if (cursor->Info)
							{
							if (cursor->Info->Rank>0) sprintf(OSTR,"%s|%s",cursor->Info->RankName,cursor->Info->Name);
							else SCopy(cursor->Info->Name,OSTR);
...
void C4Game::KeyboardInput(WORD vk_code, BOOL fAlt)
  {
	...
	if (fAlt)
		...
		switch (vk_code)
			...
			case VK_F5: Toggle(Config.General.DebugMode); if (!Config.General.DebugMode) GraphicsSystem.DeactivateDebugOutput(); break;
			}
		// Debug output toggle
