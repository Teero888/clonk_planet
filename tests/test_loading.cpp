...
int main(int argc, char** argv) {
    ...
    if (magic[0] == 0x1f && magic[1] == 0x8b) {
        ...
        char cmd[512];
        sprintf(cmd, "cp %s %s.tmp.gz && python3 -c \"d=open('%s.tmp.gz','rb').read(); open('%s.tmp.gz','wb').write(bytes([d[0]^1, d[1]^7])+d[2:])\" && gzip -df %s.tmp.gz", filename, filename, filename, filename, filename);
        system(cmd);
        char tmpname[512];
    ...
    printf("Entry 0 hex: ");
    for (int i=0; i<32; i++) printf("%02x ", entry0[i]);
    printf("\n");
